...
//                 fail_cnt = fail_cnt + 1;
//                 $display("[FAIL] %0t : %0s -> state expected=%0d actual=%0d",
//                          $time, tag, expected, dut.c_state);
//             end else begin
//                 pass_cnt = pass_cnt + 1;
//                 $display("[PASS] %0t : %0s -> state=%0d", $time, tag,
//                          dut.c_state);
//             end
//         end
//     endtask
...
//                 fail_cnt = fail_cnt + 1;
//                 $display("[FAIL] %0t : %0s -> expected=%0b actual=%0b", $time,
//                          tag, expected, actual);
//             end else begin
//                 pass_cnt = pass_cnt + 1;
...
//     task check_val(input [1:0] actual, input [1:0] expected,
//                    input [8*100-1:0] tag);
//         begin
//             if (actual !== expected) begin
//                 fail_cnt = fail_cnt + 1;
//                 $display("[FAIL] %0t : %0s -> expected=%0d actual=%0d", $time,
//                          tag, expected, actual);
//             end else begin
//                 pass_cnt = pass_cnt + 1;
...
//         end
//         check_val(o_digit_cho, 2'b00,
//                   "T4 4번 눌러 digit_cho가 0->1->2->3->0 로 wrap");
...
//         check_bit(o_start, 1'b0,
//                   "T5 START 진입 직후 o_start 아직 0 (1클럭 지연)");
//         i_start = 0;
//         @(posedge clk);
//         #1;
//         check_state(START, "T5 START 유지 (i_zero/i_start 없음)");
//         check_bit(o_start, 1'b1,
//                   "T5 o_start=1 로 올라옴 (카운트다운 enable)");
//         @(posedge clk);
//         #1;
...
//         check_bit(o_start, 1'b1,
//                   "T6 DONE 진입 직후 o_start 아직 1 (1클럭 지연)");
//         i_zero = 0;
//         @(posedge clk);
//         #1;
...
//         #1;
//         check_state(DOWN,
//                     "T8 i_up=0, 나머지 동시 -> i_down 우선 -> DOWN");
//         i_down   = 0;
//         i_beside = 0;
//         i_start  = 0;
//         @(posedge clk);
...
//         #1;
//         check_state(BESIDE,
//                     "T8 i_beside,i_start 동시 -> i_beside 우선 -> BESIDE");
//         i_beside = 0;
//         i_start  = 0;
//         @(posedge clk);
//         #1;
...
//         i_up = 1;
//         i_down = 1;
//         i_beside = 1;  // DONE에서는 전부 무시되어야 함
//         @(posedge clk);
//         #1;
//         check_state(
//             STOP,
//             "T10 DONE은 i_up/i_down/i_beside 무시하고 무조건 STOP");
//         i_up = 0;
//         i_down = 0;
//         i_beside = 0;

//         $display("==============================================");
//         $display(" [1/3] tb_control_unit_2 RESULT : PASS=%0d FAIL=%0d",
//                  pass_cnt, fail_cnt);
//         $display("==============================================");
//         $finish;
//     end
...
//                 fail_cnt = fail_cnt + 1;
//                 $display("[FAIL] %0t : %0s -> expected=%0d actual=%0d", $time,
//                          tag, expected, o_digit_pos);
//             end else begin
//                 pass_cnt = pass_cnt + 1;
//                 $display("[PASS] %0t : %0s -> pos=%0d", $time, tag,
//                          o_digit_pos);
//             end
//         end
//     endtask
...
//         @(posedge clk);
//         #1;
//         check_pos(2'd1, "T4 left 1회 -> pos=1");
//         @(posedge clk);
//         #1;
//         check_pos(2'd2, "T4 left 2회 -> pos=2");
//         @(posedge clk);
//         #1;
//         check_pos(2'd3, "T4 left 3회 -> pos=3");
//         @(posedge clk);
//         #1;
//         check_pos(2'd3, "T4 left 4회 -> 포화, 여전히 pos=3");
//         i_left = 0;
//         @(posedge clk);
//         #1;
...
//         @(posedge clk);
//         #1;
//         check_pos(2'd2, "T5 right 1회 -> pos=2");
//         @(posedge clk);
//         #1;
//         check_pos(2'd1, "T5 right 2회 -> pos=1");
//         @(posedge clk);
//         #1;
//         check_pos(2'd0, "T5 right 3회 -> pos=0");
//         @(posedge clk);
//         #1;
//         check_pos(2'd0, "T5 right 4회 -> 포화, 여전히 pos=0");
//         i_right = 0;
//         @(posedge clk);
//         #1;
...
//         $display("==============================================");
//         $display(" [2/3] tb_clock_control_unit RESULT : PASS=%0d FAIL=%0d",
//                  pass_cnt, fail_cnt);
//         $display("==============================================");
//         $finish;
//     end
...
module tb_top_stopwatch_clock;
    ...
    task check_eq(input [31:0] actual, input [31:0] expected,
                  input [8*140-1:0] tag);
        begin
            if (actual !== expected) begin
                fail_cnt = fail_cnt + 1;
...
//                  "B0 타임어택 모드 진입, control_unit2=STOP");
...
//         press(btn_R);
//         check_eq(uut.w_digit_cho, 1,
//                  "B2 beside 1회 -> digit_cho=1(sec 선택)");
//         press(btn_U);
//         press(btn_U);
...
//         // start : 카운트다운 시작
//         press(btn_L);
//         check_eq(uut.U_CONTROL_UNIT_2.c_state, ST_START,
//                  "B4 start 후 control_unit2=START");
//         repeat (200) @(posedge clk);
//         snap1 = uut.U_STOPWATCH_DATAPATH.U_MSEC_COUNTER.counter_reg;
//         check_true(snap1 != 3,
//                    "B5 카운트다운 진행중 msec 값이 변함(감소)");

//         // 일시정지 (start 재입력) -> 값 고정
//         press(btn_L);
//         check_eq(uut.U_CONTROL_UNIT_2.c_state, ST_STOP,
//                  "B6 카운트 중 start 재입력 -> STOP(일시정지)");
//         snap1 = uut.U_STOPWATCH_DATAPATH.U_MSEC_COUNTER.counter_reg;
//         repeat (200) @(posedge clk);
...
//                 fail_cnt = fail_cnt + 1;
//                 $display(
//                     "[FAIL] %0t : B9 TIMEOUT - w_zero가 끝내 1이 되지 않음",
//                     $time);
//                 disable wait_zero;
//             end
//         join
...
//         check_eq(uut.w_ta_start, 0,
//                  "B11 STOP 복귀 후 카운트다운 enable(o_start)=0");
...
//         check_true(uut.w_c_msec != snap1,
//                    "C4 set_mode에서 btn_U -> 선택된 자리 값 증가");

//         snap1 = uut.w_c_msec;
//         press(btn_D);  // 값 감소
//         check_true(uut.w_c_msec != snap1,
//                    "C5 set_mode에서 btn_D -> 선택된 자리 값 감소");
...
//         check_eq(
//             uut.w_c_digit_pos, 1,
//             "C7 set_mode=0 에서는 digit_pos 이동 안 함(그대로 1)");
...
//         repeat (5) @(posedge clk);
//         check_eq(uut.U_CONTROL_UNIT_2.c_state, ST_STOP,
//                  "D0 clock 모드 진입 시 control_unit2=STOP");
//         press(btn_L);
//         press(btn_U);
//         press(btn_R);
//         press(btn_D);
...
//                  "D2 clock 모드에서 버튼 입력해도 control_unit(run_stop) 상태 불변");
        ...
        $display("==============================================");
        $display(" [3/3] tb_top_stopwatch_clock RESULT : PASS=%0d FAIL=%0d",
                 pass_cnt, fail_cnt);
        $display("==============================================");
        $finish;
    end
