...
//
//  [무작위로 만드는 것]
//    1) 바이트 열 : 절반 남짓은 위 12개 명령 중 무작위, 나머지는 완전
//       무작위 바이트(개행, 숫자, 소문자 등 매핑 안 된 문자)
//    2) FIFO 가 잠깐 비는 구간을 무작위로 끼워 넣는다 (rx_empty=1)
//
//  [검사]
//    - pop 이 일어난 그 클럭에 그 바이트에 해당하는 명령만 1 인가
//      (12개 중 정확히 하나. 나머지 11개는 0)
...
module tb_ascii_decoder ();
    ...
    always @(posedge clk) begin
        if (monitor_on && !reset) begin
            if (rx_pop) begin
                ...
                expv = 12'b0;
                chk(cmd_vec === 12'b0, "command pulse without pop");
            end
        end
    end
    ...
    initial begin
        ...
        for (i = 0; i < N_BYTE; i = i + 1) begin
            if (({$random(seed)} % 10) < 6) arr[i] = cmd_tbl[{$random(seed)} % 12];
