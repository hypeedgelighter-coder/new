...
`ifdef RUN_STOPWATCH
        ...
        $display("  현재값 %0d:%0d:%0d.%0d", sw_hour, sw_min, sw_sec,
                 sw_msec);
`endif
...
`ifdef RUN_WATCH
        ...
        uart_send("U");
        repeat (200) @(posedge clk);
        $display("  시계값 %0d:%0d:%0d  (min 이 3 이어야 함)", wt_hour,
                 wt_min, wt_sec);
`endif
...
`ifdef RUN_SR04
        ...
        end else $display("  OK : BTNC/global reset clears SR04");
        reset = 1'b0;
        ...
        if (DUT.w_sr04_valid !== 1'b0 || DUT.w_distance !== 9'd0) begin
            err = err + 1;
            $display(
                "  ** FAIL : stuck-high echo was accepted (distance=%0d valid=%0b)",
...
`ifdef RUN_DHT11
        ...
        if (dht_valid_raw !== 1'b1) begin
            ...
            err = err + 1;
            $display(
                "  ** FAIL : 프레임 종료 뒤 버스가 High로 복귀하지 않음");
        end else $display("  OK");
