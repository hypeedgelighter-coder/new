...
//=====================================================================
// tb_uart_comm  -  PC <-> FPGA UART 전 구간 테스트벤치
//
//   PC 역할을 TB 가 한다.
...
module tb_uart_comm;
    ...
    reg full_d = 0;
    always @(posedge clk) full_d <= DUT.w_tx_full;
    ...
    task pulse_sr04;
        begin
            @(negedge clk); sr04_done = 1;
            @(negedge clk); sr04_done = 0;
        end
    endtask
    ...
    initial begin
        $display("");
        $display("==================================================");
        $display("  tb_uart_comm   baud=%0d  1bit=%0dns", BAUD, BIT_NS);
        $display("==================================================");
